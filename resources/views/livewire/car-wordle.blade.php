                    ...
                    <div class="relative bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-sm sm:w-full sm:p-6">
                        <div>
                            ...
                            <div class="mt-3 text-center sm:mt-5">
                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">{{ __('You are correct') }}</h3>
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">{{ __('The car was indeed :carName. come back tomorrow for a new car and a new challenge', ['carName' => app()->getLocale() === 'ar' ? $car->name_ar : $car->name ]) }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="mt-5 sm:mt-6">
                        <a href="/" class="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:text-sm">{{ __('Homepage') }}</a>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-end sm:items-center justify-center min-h-full p-4 text-center sm:p-0">
                <div class="relative bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-sm sm:w-full sm:p-6">
                    <div>
                        ...
                        <div class="mt-3 text-center sm:mt-5">
                            <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">{{ __('You failed this challenge') }}</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">{{ __('You should refresh the page and try again') }}</p>
                            </div>
                        </div>
                    </div>
                    <div class="mt-5 sm:mt-6">
                        <a href="/" type="button" class="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:text-sm">{{ __('Try again') }}</a>
                    </div>
                </div>
            </div>
        </div>
